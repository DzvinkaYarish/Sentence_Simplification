...
public class AnalysisUtilities {
	...
	public static void downcaseFirstToken(Tree inputTree) {
		...
		if (!preterm.label().value().matches("^NNP.*")
				&& !firstWord.equals("I")) {
			//if(firstWord.objectIndexOf('-') == -1 && !firstWord.equals("I")){
		...
		if (parser == null) {
			try {
				...
				String serializedInputFileOrUrl = ClassLoader.getSystemResource(GlobalProperties.getProperties().getProperty(
						"parserGrammarFile",
						"config/englishPCFG.ser.gz")).toExternalForm();
				System.out.println(serializedInputFileOrUrl);
	...
	public static boolean filterOutSentenceByPunctuation(String sentence) {
		//return (sentence.objectIndexOf("\"") != -1 
		//|| sentence.objectIndexOf("''") != -1 
		//|| sentence.objectIndexOf("``") != -1
		//|| sentence.objectIndexOf("*") != -1);
	...
	/**
	 * outputs the label values. This is used to replace the .yield(){
				e.printStackTrace();
			}.toString() construct that was
